...
describe('BlueprintComponent', () => {
  ...
  it('should render the rooms list correctly', () => {
    ...
    expect(
      roomListItems[0].querySelector('.room-label')?.textContent,
    ).toContain('rooma');
    expect(
      roomListItems[0].querySelector('.item-count')?.textContent,
    ).toContain('2 items');
    expect(
      roomListItems[1].querySelector('.room-label')?.textContent,
    ).toContain('roomb');
    expect(
      roomListItems[1].querySelector('.item-count')?.textContent,
    ).toContain('1 items');
  });
  ...
  it('should render the blueprint grid with rooms', () => {
    ...
    expect(gridCells[1].querySelector('.cell-name')?.textContent).toContain(
      'roomb',
    );
